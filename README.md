...
- `docs/releases/V5_0_0_OFFICIAL_2026-03-14.md`（official baseline）
...
- 提供 Memory v5 scoped memory（`agent_id / user_id` 隔离）
...
- 把 `memory_v5_maintenance.py` 从“原始审计脚本”推进成更稳定的默认巡检入口
- 默认报告补齐：
  - `status.level`
  - `alerts[]`
  - `hot_scopes[]`
  - `recommendations[]`
- 支持通过 `memory_v5.lifecycle_alerts.*` 调整 lifecycle 告警阈值
- 保留 `--no-alerts` 纯 report-first 模式，避免 silent runtime drift
