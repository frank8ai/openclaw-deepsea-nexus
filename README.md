<<<<<<< HEAD
# 🧠 Deep-Sea Nexus v5.2.0
=======
# 🧠 OpenClaw Deep-Sea Nexus v5.2.0
...
- 保留 `v5.0.1` 的 lifecycle operator 增强与 report-first 维护路径
- 新增内部 `runtime_middleware` 插件：
  - RTK 风格工具输出压缩
  - `tool_event` 结构化写入 `Memory v5`
  - `token_before / token_after / reduction_ratio / salience_score`
  - OpenClaw `tool-call` 首个适配入口
- 新增 middleware 运行态可观测性：
  - `health --json` 暴露 plugin summary
  - `paths --json` 暴露 middleware metrics
  - `scripts/runtime_middleware_report.py`
- 收敛 Windows / shell / path 兼容性问题，`tests.test_memory_v5` 全量通过
